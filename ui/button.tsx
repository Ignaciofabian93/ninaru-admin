...
export default function Button({ text, onClick, style = "", type = "primary" }: Button) {
  const buttonType = {
    ...
    secondary: "bg-white border-[1px] border-black text-slate-950",
    warning: "bg-red-700 border-[1px] border-transparent text-white",
  };
  return (
    <button
      className={`${buttonType[type]} min-w-[120px] transition-all duration-300 ease-in-out text-white py-2 px-4 rounded ${style}`}
      onClick={onClick}
    >
      {text}
    </button>
